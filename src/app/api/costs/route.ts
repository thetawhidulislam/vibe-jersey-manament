...
import { costSchema } from "@/lib/validations/entry";

const handlers = createCrudHandlers(prisma.cost as any, (body) => { const parsed = costSchema.safeParse(body); return parsed.success ? { success: true, data: parsed.data } : { success: false, error: parsed.error.flatten() }; });
